#!/usr/bin/env python3
"""
UNFINISHED — not runnable in its current state.

This node was part of a person re-identification module that was never
completed. It depends on three modules (hsvHistogramCode, publish_image and
compareFrame) that were not preserved. It is published as a record of the
approach taken, not as working code.

See wip/README.md for details.
"""
...
class YoloVisualization:
	def __init__(self):
		...
		self.base_path = rospy.get_param('~base_path', os.path.expanduser('~/imgtest/results'))
