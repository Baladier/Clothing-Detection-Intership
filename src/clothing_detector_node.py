...
class CLOTHING_DETECTOR:
    def __init__(self):
        ...
        self.IOU = rospy.get_param('~iou', 0.4)
        self.DETECTION_WIDTH = rospy.get_param('~detection_width', 640)

        # Load the clothing detection model
        models_dir = os.path.join(rospack.get_path('clothing_detection'), 'models')
        os.makedirs(models_dir, exist_ok=True)
        model_path = os.path.join(models_dir, 'yolov3-modanet.opt.onnx.prototxt')
        weight_path = os.path.join(models_dir, 'yolov3-modanet.opt.onnx')
